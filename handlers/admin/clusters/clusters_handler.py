...
@router.callback_query(AdminClusterCallback.filter(F.action == "change_domain"), IsAdminFilter())
async def request_new_domain(callback_query: AdminClusterCallback, state: FSMContext):
    """Запрашивает у администратора новый домен."""
    await state.set_state(AdminClusterStates.waiting_for_new_domain)
    await callback_query.message.edit_text(
        text="🌐 Введите новый домен (без https://):\nПример: pocomachodomen.ru",
    )


@router.message(AdminClusterStates.waiting_for_new_domain)
